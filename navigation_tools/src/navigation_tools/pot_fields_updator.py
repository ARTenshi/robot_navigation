...
def param_updator():
    ...
    if rospy.has_param(pot_fields_d0) and rospy.has_param(pot_fields_k_rej):
        ...
        rospy.logerr(f"PotFieldsUpdator.->pot_fields parameter does not exist")

    rospy.logwarn(f"PotFieldsUpdator.->Waiting for pot_fields_update msg received!!!")

    while not rospy.is_shutdown():
        update_flag = rospy.wait_for_message('/navigation/pot_fields_update', Bool)

        if update_flag.data is (True or true):
            rospy.logwarn(f"PotFieldsUpdator.->Update Parameter PotFields: d0->0.5, k_rej->2.0")
            ...
            rospy.logwarn(f"PotFieldsUpdator.->Update Parameter PotFields: d0->0.25, k_rej->0.35")
